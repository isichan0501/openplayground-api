...
class Auth:
    ...
    def verify_otp_code(self, otp_code):
        ...
        r = self.session.post(
            self.api_url + self.api_url_sia + "/attempt_first_factor", data=payload)
        data = r.json()
        self.check_errors(r)
    ...
    def login_part_1(self, email_address):
        warnings.warn(
            "Auth.login_part_1 is deprecated, use Auth.send_otp_code instead.")
        return self.send_otp_code(email_address)

    def login_part_2(self, code):
        warnings.warn(
            "Auth.login_part_2 is deprecated, use Auth.verify_otp_code instead.")
